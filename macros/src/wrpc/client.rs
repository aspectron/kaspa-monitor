            ...
            targets.push(quote! {
                    ...
                    Self: 'async_trait,
                {
                    Box::pin(async move {
                        ...
                        let __self = self;
                        //let request = request;
                        let __ret: RpcResult<workflow_serializer::prelude::Serializable<#response_type>> = {
                            let resp: ClientResult<workflow_serializer::prelude::Serializable<#response_type>> = __self.inner.rpc_client.call(#rpc_api_ops::#handler, workflow_serializer::prelude::Serializable(request)).await;
                            Ok(resp.map_err(|e| kaspa_monitor_rpc_core::error::Error::RpcSubsystem(e.to_string()))?)
                        };
                        #[allow(unreachable_code)]
                        __ret.map(workflow_serializer::prelude::Serializable::into_inner)
                    })
                }
